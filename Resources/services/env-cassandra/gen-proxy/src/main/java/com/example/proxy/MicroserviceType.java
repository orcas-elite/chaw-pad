...
public abstract class MicroserviceType {
	...
	public ResponseEntity<String> getInstanceLogin() {
		
		Integer delay = requestDelay("getInstanceLogin");
	try {
		Thread.sleep(delay);
	...
	restTemplate.getForObject("http://app:8080/auth", String.class);
		return new ResponseEntity<String>("Operation getInstanceLogin executed successfully.", HttpStatus.OK);
	}
	@RequestMapping(value = "/order", method = GET)
	public ResponseEntity<String> getInstanceOrder() {
		
		Integer delay = requestDelay("getInstanceOrder");
	try {
		Thread.sleep(delay);
	} catch(InterruptedException ie) {
		System.out.println("Exception occurred while trying to inject delay of " + delay + ". (" + ie.getMessage() + ")");
	...
	restTemplate.getForObject("http://app:8080/process", String.class);
		return new ResponseEntity<String>("Operation getInstanceOrder executed successfully.", HttpStatus.OK);
	}
}
