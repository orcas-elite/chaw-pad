...
public abstract class MicroserviceType {
	protected String type = "restdata";
	...
	public ResponseEntity<String> write() {
		...
		Integer delay = requestDelay("write");
	try {
		Thread.sleep(delay);
	...
	restTemplate.getForObject("http://cassandra:8080/store", String.class);
		return new ResponseEntity<String>("Operation write executed successfully.", HttpStatus.OK);
	}
	@RequestMapping(value = "/read", method = GET)
	public ResponseEntity<String> read() {
		
		Integer delay = requestDelay("read");
	try {
		Thread.sleep(delay);
	} catch(InterruptedException ie) {
		System.out.println("Exception occurred while trying to inject delay of " + delay + ". (" + ie.getMessage() + ")");
	...
	restTemplate.getForObject("http://cassandra:8080/read", String.class);
		return new ResponseEntity<String>("Operation read executed successfully.", HttpStatus.OK);
	}
}
