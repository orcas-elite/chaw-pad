...
public abstract class MicroserviceType {
	...
	public ResponseEntity<String> getIndexLogin() {
		
		Integer delay = requestDelay("getIndexLogin");
	try {
		Thread.sleep(delay);
	...
	restTemplate.getForObject("http://proxy:8080/login", String.class);
		return new ResponseEntity<String>("Operation getIndexLogin executed successfully.", HttpStatus.OK);
	}
	@RequestMapping(value = "/order", method = GET)
	public ResponseEntity<String> getIndexOrder() {
		
		Integer delay = requestDelay("getIndexOrder");
	try {
		Thread.sleep(delay);
	} catch(InterruptedException ie) {
		System.out.println("Exception occurred while trying to inject delay of " + delay + ". (" + ie.getMessage() + ")");
	...
	restTemplate.getForObject("http://proxy:8080/order", String.class);
		return new ResponseEntity<String>("Operation getIndexOrder executed successfully.", HttpStatus.OK);
	}
}
