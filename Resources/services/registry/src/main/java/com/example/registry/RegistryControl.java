...
public class RegistryControl implements ServiceApi, ControlApi {
    ...
    @Override
    @RequestMapping(value = "/realAnomaly", method = POST)
    public void addAnomaly(
            @RequestBody MicroserviceRealAnomalyInjection anomalyInjection) {

        String key;
        String type;
        if(anomalyInjection instanceof MicroserviceRealServiceLevelAnomalyInjection) {
            MicroserviceRealServiceLevelAnomalyInjection tmp = (MicroserviceRealServiceLevelAnomalyInjection)anomalyInjection;
            key = tmp.getKey();
            type = "SVC";
        } else if(anomalyInjection instanceof MicroserviceRealInstanceLevelAnomalyInjection) {
            MicroserviceRealInstanceLevelAnomalyInjection tmp = (MicroserviceRealInstanceLevelAnomalyInjection)anomalyInjection;
            key = tmp.getKey();
            type = "INS";
        } else if(anomalyInjection instanceof MicroserviceRealMethodLevelAnomalyInjection) {
            MicroserviceRealMethodLevelAnomalyInjection tmp = (MicroserviceRealMethodLevelAnomalyInjection)anomalyInjection;
            key = tmp.getKey();
            type = "MET";
        } else {
            key = anomalyInjection.getKey();
            type = "";
        }
        ...
        System.out.println("anomalyDelay {type: " + anomalyInjection.getServiceType() + "." + type + ", uuid: " + anomalyInjection.getUuid() + ", method: " + anomalyInjection.getMethod() + ", duration: " + anomalyInjection.getDuration() + ", start: " + start + ", end: " + end + ", delay: " + anomalyInjection.getOffset() + "}");
